...
class MarketDataCleaningPipeline:
    ...
    def run(
        ...
        *,
        ...
        start = verified[0].isyatirim.request_start_date
        end = verified[0].isyatirim.request_end_date
        request = SnapshotRequest(
            source="cleaning",
            dataset_type=self.config.cleaning.clean_dataset_type,
            ticker_or_instrument=tickers[0] if len(tickers) == 1 else "BIST_BATCH",
    ...
    @staticmethod
    def _validate_batch(values: Sequence[_VerifiedInputs]) -> None:
    ...
    def _prepare_nominal(frame: pd.DataFrame, ticker: str) -> pd.DataFrame:
        ...
        result = frame[["ticker", "date", *NOMINAL_OHLC_COLUMNS]].copy()
